...
export default function BankAccountsTab() {
  ...
  useEffect(() => {
    async function loadAccounts() {
      const data = await getVaultData('bank_accounts', defaultAccounts);
      setAccounts(data);
    }
    loadAccounts();
  }, []);
  ...
  const sync = async (data) => {
    setAccounts(data);
    await setVaultData('bank_accounts', data, true);
  ...
  const handleDelete = async (id) => {
    ...
    if (!window.confirm('Delete this bank/cash account record?')) return;
    await sync(accounts.filter(a => a.id !== id));
  };
  ...
  const handleSaveAccount = async (e) => {
    ...
    if (editItem) {
      await sync(accounts.map(a => a.id === editItem.id ? { ...form, id: editItem.id } : a));
    } else {
      await sync([...accounts, { ...form, id: Date.now() }]);
    }
    setModalOpen(false);
  };
    ...
    <div className="flex flex-col gap-5 max-w-5xl select-none text-slate-100">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Manage church bank ledgers, UPI merchant endpoints, and emergency cash vaults.</p>
        </div>
        <button 
          type="button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {accounts.map(acc => (
          <div key={acc.id} className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.08] flex flex-col justify-between gap-4 hover:border-cyan-500/30 transition shadow-lg">
          ...
          <div className="w-full max-w-md bg-slate-900 border border-white/10 rounded-2xl p-5 shadow-2xl animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-4">
              <h4 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                <span>{editItem ? 'Edit Account Ledger' : 'Add Bank Account / Cash Vault'}</span>
              </h4>
